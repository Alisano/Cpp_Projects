...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int j = 0; j < THREAD_COUNT; j++) {
            ...
            vptr_args[j].j = j;
            vptr_args[j].objects = objects;
            pthread_create(&thread[j], NULL, thread_func, (void*)&vptr_args[j]);
        }
        ...
        for (int j = 0; j < THREAD_COUNT; j++) {
            for (int i = current_index + step * j; i < current_index + step * (j + 1); i++) {
                pos_fall_ball[i - current_index][0] = vptr_args[j].objects[i].pos_of_circle_or_orth_of_line[0];
                pos_fall_ball[i - current_index][1] = vptr_args[j].objects[i].pos_of_circle_or_orth_of_line[1];
                vel_fall_ball[i - current_index][0] = vptr_args[j].objects[i].vel_fall_ball[0];
